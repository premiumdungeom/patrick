...
def register_handlers(dispatcher):
    ...
    dispatcher.add_handler(MessageHandler(Filters.regex("🆕"), new_task))
    dispatcher.add_handler(MessageHandler(Filters.regex("💰"), account))
    dispatcher.add_handler(MessageHandler(Filters.regex("👑"), claim_ptrst))
    dispatcher.add_handler(MessageHandler(Filters.regex("👫"), friends))
    dispatcher.add_handler(MessageHandler(Filters.regex("⛏️"), claim_ton))
    dispatcher.add_handler(MessageHandler(Filters.regex("🛠️"), lambda u, c: u.message.reply_text("we are on a very long journey")))
    dispatcher.add_handler(MessageHandler(Filters.regex("💘"), admin_panel))

    dispatcher.add_handler(MessageHandler(Filters.regex("📤"), trigger_withdraw))
    dispatcher.add_handler(MessageHandler(Filters.text & ~Filters.command, withdraw_request))
    dispatcher.add_handler(MessageHandler(Filters.regex("🏮SET_WALLET"), wallet_handler))
    dispatcher.add_handler(MessageHandler(Filters.text & ~Filters.command, wallet_handler))
    dispatcher.add_handler(MessageHandler(Filters.regex("🚏BACK"), start))

    dispatcher.add_handler(MessageHandler(Filters.regex("🚀"), total_user))
    dispatcher.add_handler(MessageHandler(Filters.regex("💱"), total_payout))
    dispatcher.add_handler(MessageHandler(Filters.regex("🍍"), set_new_task))
    dispatcher.add_handler(MessageHandler(Filters.text & ~Filters.command, handle_task_text))
    dispatcher.add_handler(MessageHandler(Filters.regex("👗"), broadcast))
